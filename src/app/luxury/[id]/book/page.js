...
/**
 * Review Booking for a luxury experience.
 *
 * The package is charged per guest, children included, and the price summary
...
export default async function Page({ params, searchParams }) {
  ...
  const taxes = Math.round(paid * luxuryTaxRate);
  ...
  return (
    <div className="pb-36 lg:pb-12">
      <ScreenBar title="Review Booking" backHref={`/luxury/${s.id}`} />
      ...
      <div className="shell py-4">
        <BookingForm
          ...
          confirm={{ kind: 'luxury', name: s.name, slot: fullDate(date), checkIn: date, nights: clock(time), location: where }}
        >
          <section className="card overflow-hidden">
            ...
            <div className="border-t-4 border-surface-soft p-4 sm:p-5">
              <p className="text-[15px] font-semibold text-ink-900">Package</p>
